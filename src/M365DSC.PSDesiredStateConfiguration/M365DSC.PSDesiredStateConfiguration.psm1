...
# The engine compiles every configuration body into a call to the module-qualified
# name 'PSDesiredStateConfiguration\Configuration' and resolves it by name at
# invocation time. This module ships under its own name, so it claims that literal
# function name globally; without it the call would go to the inbox 1.1 (Windows
# PowerShell) or gallery 2.x (PowerShell 7) module instead. Parsing a configuration
# makes the engine load the inbox module by file path, which takes the name over
# again, so the claim is re-asserted after every parse and before every invocation.
function Assert-DscConfigurationShim
{
    [OutputType([void])]
    param()

    ${function:global:PSDesiredStateConfiguration\Configuration} = ${function:Configuration}
}
